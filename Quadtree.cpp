...
private:
    Image* image;
    QuadNode* root;      // ADDED: Keep track of the root for reconstruction
    int rootSize;        // ADDED: Keep track of the starting size
    int totalNodes;
    int leafNodes;
    int threshold;  
    ...
    bool isUniform(int x, int y, int size) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                if (abs(pixel - firstPixel) > threshold) {
                    return false;  
                }
            }
        }
    ...
    // ADDED: Helper to paint the blocks back onto a canvas
...
public:
    ...
    // ADDED: Create the visual reconstruction image
